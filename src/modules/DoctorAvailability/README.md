...
* Check the `ConnectionStrings` in `appsettings.json` files under the `Doctor.Availability.HttpApi.Host` and `Doctor.Availability.DbMigrator` projects and change it if you need.

### Before running the application

* Run `abp install-libs` command on your solution folder to install client-side package dependencies. This step is automatically done when you create a new solution, if you didn't especially disabled it. However, you should run it yourself if you have first cloned this solution from your source control, or added a new client-side package dependency to your solution.
* Run `Doctor.Availability.DbMigrator` to create the initial database. This step is also automatically done when you create a new solution, if you didn't especially disabled it. This should be done in the first run. It is also needed if a new database migration is added to the solution later.
...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p 7647eb4c-bb49-41af-88de-6db6d7cf78af
```

> `7647eb4c-bb49-41af-88de-6db6d7cf78af` is the password of the certificate, you can change it to any password you want.
...
* `Doctor.Availability.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
* `Doctor.Availability.HttpApi.Host`: ASP.NET Core API application that is used to expose the APIs to the clients.
